...
import { putUserCountry, putUserCurrency } from "../../../../../api/helpers/requests/user";
...
export default function ChangeCountryModal(props: IChangeCountryModal) {
    ...
    const renderCountries = (countries: Country[]) => {
        return countries.map((country: Country, index) => {
            return (
                <option key={index} value={country.country_id}>{country.code}</option>
            )
        })
    }

    const handleChange = (option) => {
        const country = countries.filter(country => country.country_id === parseInt(option.target.value))[0]
